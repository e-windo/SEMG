%Script to visually compare experimental data with simulated data
sensorIDX = 8;
...
%figure;
%subplot(2,1,1)
plotTimeFreq(sosfilt(Hd.sosmatrix,analysis{2}(1:length(X))),2000)
%figure;
%subplot(2,1,2)
construct = X+(Y(1:length(X))+circshift(Y(1:length(X)),1200))*0.4+0.2*Z(1:length(X));
plotTimeFreq(sosfilt(Hd.sosmatrix,construct),2000);
ylim([0,0.02]);
%}
