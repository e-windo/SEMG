function words = num2words(number)   
	%num2words returns the pretty printed wordsy form of any number less than about 10^63
	
    if ~isvector(number)
        error('Number has to be a vector');
    end
    words = cell(size(number));
    for i = 1:length(number)
        %% integer part
        int_number = floor(number(i));
        temp = int2words(int_number);
        %% digits after the decimal point
        if int_number ~= number(i)
            %% base words
...
function words = int2words(number)
    ...
    minus     = 'minus';
    names0    = 'zero';
    names1000 = {'', '_thousand', '_million', '_billion', '_trillion', ...
                 '_quadrillion', '_quintillion', '_sextillion', ...
                 '_septillion', '_octillion', '_nonillion', ...
                 '_decillion', '_undecillion', '_duodecillion', ...
                 '_tredecillion', '_quattuordecillion', ...
                 '_quindecillion', '_sexdecillion', '_septendecillion', ...
                 '_ocotdecillion', '_novemdecillion', '_vigintillion'};
...
function words = int2words_1to999(number)
    ...
    if number > 0
        ...
        words = [names1{number+1}, '_hundred', words];
    end
end